...
import { attachSimpleLoginAliasSyncStore } from "../alias";
import {
  CredentialAlgorithm,
  Profile,
  GeneratorMetadata,
        ...
        ([request, metadata]) => {
          ...
          const settings$ = settingsSubject.pipe(
            ...
            concatMap(async (settings) => {
              ...
              return attachSimpleLoginAliasSyncStore(
                settings,
                settingsSubject,
                account,
                this.provide.aliasSync?.cipherService,
                this.provide.aliasSync?.syncService,
              );
            }),
            share({
