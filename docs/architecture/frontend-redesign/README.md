...
Exact baseline ancestor: `5f640f3bbe94cdc47dbebf81f0c32548080b76da` (`origin/test/ui-redesign-baseline`). The dossier was imported from `dc81e422e3d0a3e06e87a5319933e0f3243ea89f` as its two documentation-only commits, without merging its `origin/main` ancestry, and its measured inventory was regenerated against the consolidated tracked tree. The upstream-sync sample remains anchored to the locally available `origin/main` and `upstream/main` at `1f881babc15eb7d3a88cad41730ce167d8e49a41`. The dossier itself changes no runtime, schema, commercial source, build, package, or application file.
