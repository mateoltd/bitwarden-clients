...
if (git(["merge-base", base, target]).trim() !== base) findings.push("history does not begin at pinned main");
...
for (const commit of commits) {
  ...
  if (!/^(?:build|chore|ci|docs|feat|fix|perf|refactor|revert|test)(?:\([^)]+\))?!?: .+/.test(subject)) {
...
if (!partial) {
  ...
  for (const commit of policy.portabilityCommits) {
    ...
    if (!targetPatchIds.has(expected)) findings.push(`portability patch not replayed exactly: ${commit}`);
  }
}

if (findings.length > 0) {
  process.stderr.write(`History verification failed\n${findings.map((item) => `- ${item}`).join("\n")}\n`);
  process.exitCode = 1;
} else {
