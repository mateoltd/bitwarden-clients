...
for (const path of protectedLicenses) {
  if (objectId(base, path) !== objectId(target, path))
    findings.push(`root license changed: ${path}`);
}
...
for (const lockPath of ["package-lock.json", "release/tooling/package-lock.json"]) {
  ...
  for (const [name, metadata] of Object.entries(after)) {
    if (
      ...
      name === "" ||
      metadata.link ||
      String(metadata.resolved ?? "").startsWith("file:")
    ) {
      continue;
    }
...
if (!/^GPL-3\.0/.test(rootPackage.license ?? ""))
  findings.push("root package is not GPL-3.0 licensed");
